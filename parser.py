...
class Parser:
    def parse(self, payload):
        ...
        for bit, field in self.bits.items():
            if 2**bit & flags:
                data[field.name] = struct.unpack_from(field.format_, payload, offset)[0]
...
class IndoorBikeDataParser(Parser):
    ...
    bits = {
        0: Field(name='more data', format_='<H'),
        1: Field(name='average speed', format_='<H', unit='km/h'),
        2: Field(name='instantaneous cadence', format_='<H', unit='rpm'),
        3: Field(name='average cadence', format_='<H', unit='rpm'),
...
class FitnessMachineStatusParser:
    flagsFormat = struct.Struct('<H')
    bits = {
        0: Field(name='more data', format_='<H'),
        1: Field(name='average speed', format_='<H', unit='km/h'),
